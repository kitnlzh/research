...
def main() -> None:
    price_path_env = os.getenv("PRICE_PATH")
    cycle_path_env = os.getenv("CYCLE_PATH")

    price_path = Path(price_path_env) if price_path_env else find_file_under_onedrive("selected_dates_price_trends_spot_1-0_2-1.xlsx")
    ...
    if not price_path.exists():
        raise FileNotFoundError(f"PRICE_PATH not found: {price_path}")
    if not cycle_path.exists():
        raise FileNotFoundError(f"CYCLE_PATH not found: {cycle_path}")

    run_root = make_run_dir(prefix="2d_piecewise_batch_2")
