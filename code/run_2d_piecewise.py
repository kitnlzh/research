...
# =========================
def main() -> None:
    run_dir = make_run_dir(prefix="2d_piecewise")
    ...
    S = df["Sdata"].dropna().tolist()[:10]  # 元コードに合わせて残す（未使用でも読み込みは維持）
    ...
    S10_mat = df_s10.values.tolist() # 10×10
    T10_mat = df_t10.values.tolist() # 10×10
    ...
    for t in T_q:
        ...
        #model.addConstr(k1[t] == 0)  #3つとも0でこていして。1はダメ
    ...
    # PowerShellの最後に値だけ出す（余計な文字なし）
    print(f"{total_deg_cost:.10f}")
    # 目的関数値だけを最後に1行で出力（余計な文字なし）
    print(f"{model.ObjVal:.10f}")
