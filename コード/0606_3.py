...
lambda_t = df['spot5'].tolist()[:48]  # 長さ48（0〜47）
...
delta_t = 0.5
B_const = 40
T = range(48)       # 通常の時間範囲（0〜47）
T_q = range(49)     # Qs, Qr, Qr2 用の時間範囲（0〜48）
B = range(8)
...
# 初期・終端制約（エネルギー保存）
#model.addConstr(Qr[0] + Qs[0] == Qr[48] + Qs[48])
...
# Qr, Qr2, Qs <= B制約（kが使えない t=48 では単独制約に）
for t in T_q:
    if t < 48:
