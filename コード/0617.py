...
expr = (
    ...
    + sum(x[t, i] / (i + 1) for t in T for i in I)
)
...
if model.status == GRB.OPTIMAL:
    ...
    # ← 追加出力
    print("\ntごとの P_sel, alpha*ΔkWh, alpha*ΔkWh2:")
    for t in T:
        b = t // 6
        delta_kWh = p * delta_kW[b].X * delta_t
        delta_kWh2 = p2 * delta_kW2[b].X * delta_t
        P_sel_t = P_sel[t].X
        print(f"t={t:02d}, P_sel={P_sel_t:.2f}, alpha*ΔkWh={alpha * delta_kWh:.2f}, alpha*ΔkWh2={alpha * delta_kWh2:.2f}")
else:
